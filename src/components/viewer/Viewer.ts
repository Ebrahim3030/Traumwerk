...
import { MaterialPresetPlugin } from "./MaterialPresetPlugin";
...
async function setupViewer(canvas: HTMLCanvasElement) {
  ...
  let presets = await viewer.addPlugin(MaterialPresetPlugin);

  (window as any).materialPresetsPlugin = presets;
